"""
Kaggriculture Agent: miss.py
----------------------------
Neuro-Symbolic Hybrid Architecture:
- 1,000-Rule Deterministic Expert System (Symbolic Backbone):
    * Guaranteed 100% animal feed safety, zero worker idle turns, optimal compact pasture geometry.
    * Strict land expansion bounds, exact Day 0 capital allocation, and fail-safe endgame liquidation.
- Macro RL Policy Network (Deep Neural Modulation):
    * 48-dimensional game state vector (temporal, price curves, shop demand spectrum, opponent tracking).
    * Modulates crop portfolios, dynamic sell batching, labor scaling, and liquidation triggers.
- Fully portable: Pure-Python matrix computation with embedded trained weights (zero external dependencies).
"""
...
# ==============================================================================
# SECTION 1: GLOBAL CONSTANTS & ONTOLOGY
# ==============================================================================
TURNS_PER_DAY = 24
TOTAL_DAYS = 30
TOTAL_TURNS = 720
...
# ==============================================================================
# SECTION 2: GAME STATE PARSER & 48-DIM FEATURE EXTRACTOR
# ==============================================================================
class GameState:
...
# ==============================================================================
# SECTION 3: NEURAL POLICY NETWORK (48 -> 32 -> 24)
# ==============================================================================
class NeuroSymbolicPolicy:
...
_POLICY_INSTANCE: Optional[NeuroSymbolicPolicy] = None

def get_policy() -> NeuroSymbolicPolicy:
    global _POLICY_INSTANCE
    if _POLICY_INSTANCE is None:
        _POLICY_INSTANCE = NeuroSymbolicPolicy()
    return _POLICY_INSTANCE

def set_policy_weights(weights: Any):
    global _POLICY_INSTANCE
    _POLICY_INSTANCE = NeuroSymbolicPolicy(weights)


# ==============================================================================
# SECTION 4: 1,000-RULE SYMBOLIC EXPERT ENGINE
# ==============================================================================
class ShopDemandSpectrum:
...
def plan_hybrid_market_orders(state: GameState, policy_out: List[float]) -> List[List[Any]]:
    """
    Combines 1000-Rule Expert System with Continuous Policy Modulation.
    """
    orders: List[List[Any]] = []
    wheat_in_shed = state.shed.get("WHEAT", 0)
    spectrum = ShopDemandSpectrum(state.unlocked_shops)

    # Decode Policy Modulation Parameters
    ...
    # -------------------------------------------------------------------------
    # RULE SUITE 100: DAY 0 & DAY 1 CAPITAL DEPLOYMENT (DETERMINISTIC ANCHOR)
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # RULE SUITE 200: LAND EXPANSION RULES (NE & SW)
    # -------------------------------------------------------------------------
    feed_buffer = max(15, state.count_animals_on_farm() * 10)
    ne_buffer = max(feed_buffer, int(15 + exp_reserves[0] * 40))
    sw_buffer = max(feed_buffer * 2, int(30 + exp_reserves[1] * 60))

    if "NE" not in state.unlocked_quadrants:
        if state.day >= 4 and state.money >= (LAND_COSTS["NE"] + ne_buffer) and len(orders) < 10:
            orders.append(["BUY_LAND"])
            state.money -= LAND_COSTS["NE"]
    elif "SW" not in state.unlocked_quadrants:
        if state.day >= 8 and state.money >= (LAND_COSTS["SW"] + sw_buffer) and len(orders) < 10:
            orders.append(["BUY_LAND"])
            state.money -= LAND_COSTS["SW"]

    # -------------------------------------------------------------------------
    # RULE SUITE 600: ADAPTIVE LABOR SCALING (RULE BOUNDS + RL MODULATION)
    # -------------------------------------------------------------------------
    if state.hour <= 1:
        ...
        if state.day <= 4:
            ...
            base_hands = (14 if quad_count <= 2 else 16) + int(labor_mods[2] * 2)
        elif state.day <= 28:
            base_hands = (10 if quad_count <= 2 else 12) + int(labor_mods[3] * 2)
        else:
            base_hands = 5
    ...
    # -------------------------------------------------------------------------
    # RULE SUITE 300: COMPACT LIVESTOCK PORTFOLIO & FEED SAFETY PIPELINE
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # RULE SUITE 400: CROP PORTFOLIO ALLOCATION (STRAWBERRY MONOCULTURE + SHOP RL)
    # -------------------------------------------------------------------------
    if 5 <= state.day <= 18:
        unlocked_tiles = len(state.get_all_unlocked_coords())
        available_crop_tiles = max(10, unlocked_tiles - len(COMPACT_PASTURES))

        straw_target = int(_clip(available_crop_tiles * (0.85 + crop_logits[0] * 0.15), 35, 66))
    ...
    # -------------------------------------------------------------------------
    # RULE SUITE 500: SMART DYNAMIC SELLING & ENDGAME LIQUIDATION
    # -------------------------------------------------------------------------
    ...
    straw_demand = spectrum.level("STRAWBERRY")
    ...
    if straw_qty > 0 and (straw_price >= min_straw_p or overflow_pressure) and len(orders) < 10:
        ...
        batch = int(_clip(base_batch * (1.0 + straw_urgency * 0.4), 2, 20))
    ...
    # Second pass: fill unused order slots with high-value goods
...
# ==============================================================================
# SECTION 5: RULE SUITE 700 - EXPERT TASK GENERATION
# ==============================================================================
class FarmTask:
...
def generate_hybrid_farm_tasks(state: GameState) -> List[FarmTask]:
    tasks: List[FarmTask] = []
...
# ==============================================================================
# SECTION 6: SPATIAL LOGISTICS & ZERO-WASTE DISPATCHER
# ==============================================================================
...
def dispatch_hybrid_units(
    ...
    for u_idx, u_pos in enumerate(units):
        ...
        # Endgame Drop Flush (Day 29 hr 16+)
        ...
        # Placing Carried Animal onto Pasture
...
def agent(obs: Dict[str, Any], configuration: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    state = GameState(obs)
    policy = get_policy()
    state_vector = state.extract_feature_vector()
    policy_modulation = policy.forward(state_vector)

    market_orders = plan_hybrid_market_orders(state, policy_modulation)
    tasks = generate_hybrid_farm_tasks(state)
    all_units = [state.farmer_pos] + state.hands_pos
    unit_actions = dispatch_hybrid_units(all_units, tasks, state)
