...
def run_50_grandmaster_benchmark():
    print("=" * 86)
    print("      KAGGRICULTURE 50-MATCH ULTIMATE GRANDMASTER ARENA BENCHMARK SUITE")
    print("=" * 86)

    matches = []
    # 20 vs Paarth Patel ($103k Architecture Clone)
    for idx, s in enumerate(range(31001, 31021)):
        p0_main = (idx % 2 == 0)
        a0 = "main.py" if p0_main else "gm_paarth_patel"
        a1 = "gm_paarth_patel" if p0_main else "main.py"
        matches.append((f"Seed {s}", a0, a1, s, "vs GM Paarth Patel ($103k)"))

    # 20 vs GM Market Monopolizer ($90k+ Scalper)
    for idx, s in enumerate(range(32001, 32021)):
        p0_main = (idx % 2 == 0)
        a0 = "main.py" if p0_main else "gm_market_monopolizer"
        a1 = "gm_market_monopolizer" if p0_main else "main.py"
        matches.append((f"Seed {s}", a0, a1, s, "vs GM Market Monopolizer ($90k)"))

    # 10 Self-Play Mirror Matches
    for s in range(33001, 33011):
    ...
    for i, (label, a0, a1, seed, match_type) in enumerate(matches, 1):
        ...
        if not is_self_play:
            ...
            print(f"[{i:02d}/50] {label} | {pos} vs {opp_name:<22} | main: ${main_m:>7,.0f} | Opp: ${opp_m:>7,.0f} | {outcome} ({res['dur']:.2f}s)")
        else:
            p0_m, p1_m = res["m0"], res["m1"]
            print(f"[{i:02d}/50] {label} | Self-Play Mirror (Elite)       | P0:   ${p0_m:>7,.0f} | P1:  ${p1_m:>7,.0f} | COMBINED ${p0_m + p1_m:>7,.0f} ({res['dur']:.2f}s)")

    print("\n" + "=" * 86)
    print("                 50-MATCH GRANDMASTER TOURNAMENT FINAL REPORT                 ")
    print("=" * 86)
    if competitive_scores:
        ...
        print(f"Opponent Peak Final Money      : ${opp_peak:,.0f}")
        print(f"Average Margin vs Grandmasters : +${avg_m - opp_avg:,.0f}")
    print("=" * 86)

if __name__ == "__main__":
    run_50_grandmaster_benchmark()
